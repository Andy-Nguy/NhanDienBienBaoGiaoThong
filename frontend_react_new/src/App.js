...
function App() {
  ...
  const handleFileChange = (event) => {
    event.stopPropagation(); // Ngăn sự kiện click lan tỏa lên upload-section
    const file = event.target.files[0];
    if (file) {
  ...
  const handleClick = (event) => {
    // Chỉ mở input file nếu click không xuất phát từ label
    if (event.target.className.includes('upload-button')) {
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const backendApiUrl = 'http://localhost:5000/predict';
      const response = await axios.post(backendApiUrl, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
        timeout: 60000,
      });
      setPredictionResult(response.data);
      setSuccessMessage('Nhận dạng thành công!');
    } catch (error) {
      console.error('Lỗi khi gửi request dự đoán:', error);
      if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
        setErrorMessage('Yêu cầu vượt quá thời gian chờ. Server có thể đang xử lý hoặc mạng chậm.');
  ...
  return (
    <div className="container">
      <header>
        ...
        <p className="subtitle">Kéo thả hoặc chọn ảnh để nhận dạng biển báo giao thông</p>
      </header>
      ...
      <main>
        ...
        >
          ...
          />
          <label htmlFor="fileInput" className="upload-button">
            {isDragging ? 'Thả ảnh vào đây' : 'Chọn Ảnh (Tối đa 5MB)'}
          </label>
        </div>
